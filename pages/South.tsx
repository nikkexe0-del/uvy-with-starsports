...
import { getSouthMovies, getSouthSeries, TMDBItem } from '../lib/tmdb';
...
export default function South() {
  ...
  useEffect(() => {
    Promise.all([getSouthMovies(), getSouthSeries()])
        ...
        <div className="relative h-[80vh] md:h-[95vh] w-full">
          ...
          <div className="absolute bottom-[20%] md:bottom-[25%] left-4 md:left-12 max-w-lg z-20">
             <div className="flex items-center gap-2 mb-4">
                <span className="px-2 py-0.5 bg-red-600 text-[10px] font-black uppercase tracking-widest text-white rounded">South Hit</span>
                <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest leading-none">Global Network</span>
             </div>
      ...
      <div className="flex flex-col gap-12 px-4 md:px-12 -mt-16 md:-mt-40 relative z-30 pb-20">
        <section>
          <h2 className="text-lg md:text-2xl font-black uppercase tracking-tighter italic mb-4 flex items-center gap-4">
             South Cinema Hits <div className="flex-1 h-[1px] bg-white/5" />
          </h2>
        ...
        <section>
          <h2 className="text-lg md:text-2xl font-black uppercase tracking-tighter italic mb-4 flex items-center gap-4">
             Trending South Shows <div className="flex-1 h-[1px] bg-white/5" />
          </h2>
          <div className="netflix-row pb-4">
            {shows.map((item) => (
              <MediaCard key={item.id} item={item} />
